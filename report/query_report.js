...
frappe.views.QueryReport = Class.extend({
	...
	render: function(res) {
		...
		if(!filters.frozenColumn){
			selectOption = []
			this.grid.getColumns().forEach((e,i)=> {
				selectOption.push({"value":i,"label":e.name});
			});

			var df = {
				"default":1,
				"fieldname": "frozenColumn",
				"fieldtype": "Select",
			...
			df.onchange = (e) => {
				...
				var selectedFilter = this.get_values(true);
				
				me.slickgrid_options.frozenColumn = +selectedFilter.frozenColumn;
				me.trigger_refresh();
			}
